...
class RobertaEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, **kwargs):
        ...
        if inputs_embeds is None:
            ...
            # logger.info("[minwoo] word_embeds[0] = {}".format(word_embeds[0]))  # 앞쪽 prompt 부분 싹다 0 으로 박힘
            #print(word_embeds.size()) #torch.Size([16, 231, 768])
            prompt_ids = - (input_ids + 1)
            # logger.info("[minwoo] input_ids[0] = {}".format(input_ids[0]))  #tensor([[ -1,  -2, ...-100,0,...,2,1,1,1],
            # logger.info("[minwoo] prompt_ids[0] = {}".format(prompt_ids[0]))  #tensor([[ 0,  1,  2,  ..., -2, -2, -2],
            #print(prompt_ids.size())  # torch.Size([16, 231]
            ...
            if "prompt_emb_output" in kwargs:
                if kwargs['prompt_emb_output']=="replace_task_specific_prompt_emb":
                    #현재 모델의 prompt_embeddings 를 사용하지를 않음!! 
                    prompt_emb = kwargs['task_specific_prompt_emb'] #projector 를 통과시킨 soft prompt
                     # logger.info(prompt_emb.size()) # torch.Size([16, 100, 1024]
                    ...
                    # logger.info(prompt_emb.size()) # torch.Size([16, 231, 1024]
                    # logger.info(prompt_emb[0]) 프롬프트 부분의 embedding vector 만 계산.
                else:
                    prompt_emb = self.prompt_embeddings(prompt_ids * (prompt_ids >= 0).int()) * (prompt_ids >= 0).int().unsqueeze(2)
            else:
                prompt_emb = self.prompt_embeddings(prompt_ids * (prompt_ids >= 0).int()) * (prompt_ids >= 0).int().unsqueeze(2)
                # logger.info("[minwoo] prompt_emb[0] = {}".format(prompt_emb[0]))  #뒤쪽 sentence token 부분 싹다 0 으로 박힘
                #prompt_ids 를 참고.
        ...
        position_embeddings[:,:100,:] = prompt_zero_position_embeddings #token 부분 0 으로 position embedding
        
        token_type_embeddings = self.token_type_embeddings(token_type_ids)

        # print("1",inputs_embeds[0])
        # print("2",inputs_embeds.size()) #2 torch.Size([64, 231, 768])
        
        # print("3",position_embeddings[0]) # 앞에 100개는 0
        # print("4",position_embeddings.size()) #4 torch.Size([64, 231, 768])
        
        # print("5",token_type_embeddings[0]) # all zero
        # print("6",token_type_embeddings.size()) #6 2 torch.Size([64, 231, 768])
        # print(self.token_type_embeddings.num_embeddings) # 1
        # exit()
        embeddings = inputs_embeds + position_embeddings + token_type_embeddings
        embeddings = self.LayerNorm(embeddings)

        self.result_embedding = embeddings
