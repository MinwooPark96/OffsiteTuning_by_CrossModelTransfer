...
def load_task_prompt(config):
    ...
    if config.get("train","prompt_emb"):
        ...
        task_prompt_emb = torch.load(filename)
        task_prompt_dict[name] = task_prompt_emb
        name_list.append(name)
        
        if config.get('distributed','local_rank') <= 0:     
            print("load source prompt from <{}> in crossPrompt.py".format(filename))
        
                        
    else :
        assert config.get("train","source_model") != False    
        for name in choosed_tasks:
            filename = "task_prompt_emb/" + name + "Prompt" + config.get("train","source_model")+"/task_prompt"    
            task_prompt_emb = torch.load(filename)
            task_prompt_dict[name] = task_prompt_emb
            name_list.append(name)
    ...
    return task_prompt_ten , name_list
...
class crossPrompt(nn.Module):
    ...
    #model(data, config, gpu_list, acc_result, "train", AE=model_AE) from train_tool_cross.py
        ...
        self.task_specific_prompt_emb, self.name_list = load_task_prompt(config)
        self.task_specific_prompt_emb = self.task_specific_prompt_emb.to("cuda")
    ...
    def forward(self, data, config, gpu_list, acc_result, mode, prompt_emb_output="replace_task_specific_prompt_emb", **kwargs):
        ...
        if data_name not in self.name_list:
            filename = "task_prompt_emb/" + data_name + "Prompt" + config.get("train","source_model")+"/task_prompt"    
            task_prompt_emb = torch.load(filename)
            task_prompt_ten = list()        
            task_prompt_dict = dict()
            
            task_prompt_dict[data_name] = task_prompt_emb
            task_prompt_ten.append(task_prompt_dict[data_name])
            task_prompt_ten = torch.stack(task_prompt_ten)
            idx_tensor = torch.tensor([0]*batch_size,dtype = torch.long).to("cuda")
            task_specific_prompt_emb = torch.index_select(task_prompt_ten, 0, idx_tensor)
            
        else :
            idx = self.name_list.index(data_name)
            idx_tensor = torch.tensor([idx]*batch_size,dtype = torch.long).to("cuda")
            task_specific_prompt_emb = torch.index_select(self.task_specific_prompt_emb, 0,idx_tensor)
