...
def drawplot_epoch_loss(length,json_name,nbins):
    ...
    for dataname in dataList:
        ...
        for j in range(1,length+1):
            ...
            labels = labels + [dataname.replace('_epoch_loss','')]
    ...
    plt.legend(frameon = True, prop = {'weight':'bold', 'size':5}, labelcolor = '0.2',loc='upper right')
    title_font = {
    'fontsize': 16,
    'fontweight': 'bold'
    }
    plt.title('Epoch Loss', fontdict=title_font, loc='left', pad=20)

    plt.savefig(json_name+"/epoch_loss.png", bbox_inches = 'tight', dpi = 250, facecolor = ax.get_facecolor())