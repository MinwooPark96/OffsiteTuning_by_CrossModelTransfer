...
def train(parameters, config, gpu_list, do_test=False, local_rank=-1, **params):
    ...
    for epoch_num in range(trained_epoch, epoch):
        ...
        total_len = min([len(dataset) for dataset in parameters['train_dataset']])
        dataset_1, dataset_2, dataset_3 = parameters['train_dataset']
        datasets_zipped = zip(dataset_1,dataset_2,dataset_3)
        ...
        #각 batch 에 대하여 
        for step, (dataset_1, dataset_2, dataset_3) in enumerate(datasets_zipped):
            # tensor to cuda
            for datas in (dataset_1, dataset_2, dataset_3): #datas = [source,target]
                for data in datas:
            ...
            if "T5" in config.get("target_model","model_base"):
                for idx,(source_dataset,target_dataset) in enumerate([dataset_1,dataset_2,dataset_3]):
        ...
        if current_epoch % test_time == 0 :
            with torch.no_grad():
                ...
                if len(parameters['valid_dataset']) == 1:
                    parameters['valid_dataset'] = [parameters['valid_dataset']] #for enumerate.
