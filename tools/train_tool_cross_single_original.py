...
import transformers
from tools.projector import AE_0_layer, AE_1_layer_mutiple_100, AE_1_layer, AE_1_layer_mutiple_100_paper,AE_transformer_layer
...
def train(parameters, config, gpu_list, do_test=False, local_rank=-1, **params):
    ...
    for epoch_num in range(trained_epoch, epoch):
        ...
        if local_rank <=0 and not "T5" in config.get("target_model","model_base"):            
            ...
            train_loss = config.get("data","train_dataset_type") + "_train_loss"
            train_valid_info[train_loss][current_epoch] = round(train_epoch_loss,4)
